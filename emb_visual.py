...
def plot_combined_sensitivity(paths, output_folder):
    ...
    for period, path in paths.items():
        if os.path.exists(path):
            data[period] = pd.read_csv(path).sort_values('Embedding_Dim')
        else:
            print(f"❌ 找不到 {period} 的文件: {path}")
            return

    # 2. 绘图参数设置
    heads = data["Morning"]['Embedding_Dim'].values
    ...
    # 6. 保存
    save_path = os.path.join(output_folder, 'emb_sensitivity_all_periods_corrected.png')
...
if __name__ == "__main__":
    file_paths = {
        "Morning": r"D:\KEKE\宣城交通流量预测\results\interp\morning\seed_42\sensitivity_analysis\embedding_sensitivity_results_corrected.csv",
        "Noon": r"D:\KEKE\宣城交通流量预测\results\interp\noon\seed_42\sensitivity_analysis\embedding_sensitivity_results_corrected.csv",
        "Evening": r"D:\KEKE\宣城交通流量预测\results\interp\evening\seed_42\sensitivity_analysis\embedding_sensitivity_results_corrected.csv"
    }
    
    output_dir = r"D:\KEKE\宣城交通流量预测\results\interp\emb_analysis_total"
