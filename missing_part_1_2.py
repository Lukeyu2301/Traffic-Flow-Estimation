...
# ================= 1. 环境配置 =================
SEED = 42
PERIOD = "evening"  # 可切换: morning / noon / evening
...
# ================= 2. 准备数据与 3 分区映射 =================
print(f">>> 正在加载 {PERIOD.upper()} 数据并读取外部预设 3 分区 SHP (1vs2 模式)...")
...
base_save_dir = os.path.join(base_path, "results", "spatial_1vs2_optimized", PERIOD)
...
# ================= 4. 1-Block Train vs 2-Blocks Test 实验循环 =================
all_blocks = [0, 1, 2]
spatial_results = []

print(f"\n>>> 启动 1-Block 训练 vs 2-Blocks 测试 极端实验...")

for train_block in all_blocks:
    test_blocks = [b for b in all_blocks if b != train_block]
    print(f"\n" + "#"*60 + f"\n[Fold {train_block+1}/3] 训练区域: Block {train_block} | 测试区域: {test_blocks}")
    
    train_idx_all = data_all[data_all['block'] == train_block].index.values
    test_idx = data_all[data_all['block'].isin(test_blocks)].index.values
    ...
    res1.update({"Model": "GAT-XGBoost", "Fold": train_block+1})
    ...
    res2.update({"Model": "XGBoost", "Fold": train_block+1})
    ...
    res3.update({"Model": "GAT-MLP", "Fold": train_block+1})
    spatial_results.append(res3)

# ================= 5. 保存结果汇总 =================
all_df = pd.DataFrame(spatial_results)
all_df.to_csv(os.path.join(base_save_dir, "spatial_1vs2_comparison_optimized.csv"), index=False)
summary = all_df.groupby("Model")[["RMSE", "MAE", "R2", "MAPE"]].agg(['mean', 'std'])
summary.to_csv(os.path.join(base_save_dir, "spatial_1vs2_summary_optimized.csv"))

print("\n" + "="*60 + f"\n📊 1-Block vs 2-Blocks 空间外推优化实验汇总 ({PERIOD.upper()})\n" + "="*60)
print(summary)